...
# Dummy in-memory "database". In production, replace with persistent storage.
fake_users_db = {}
fake_tasks_db = {}  # user_id: list of tasks

# Config settings (should be loaded from .env in production)
...
app = FastAPI(
    ...
    description="Backend for the ToDo List App. Provides endpoints for user authentication and task management.",
    version="1.0.0",
    openapi_tags=[
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allow all origins for development; restrict in prod
...
@app.post(
    ...
    description="Register a user with a username and password. Returns a JWT token upon success."
)
async def register_user(user_data: UserCreate):
    """Register a new user, hashing password and returning JWT token.
    Returns HTTP 400 if username already exists.
    """
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    """
    Authenticate user from form fields (username, password).
    Returns JWT token on correct credentials.
    """
...
async def list_tasks(
    ...
    """List and filter tasks for the authenticated user."""
...
async def get_task(
    ...
    """Retrieve one task by ID for this user or 404."""
...
@app.put(
    ...
    description="Edit an existing task's details (title, description, due date, or completion status)."
)
async def update_task(
    ...
    """Update task for user, return updated task."""
    tasks = fake_tasks_db.get(current_user.id, [])
...
async def delete_task(
    ...
    """Delete task by ID for the user. Returns 204 on success."""
    tasks = fake_tasks_db.get(current_user.id, [])
    for idx, t in enumerate(tasks):
        if t.id == task_id:
            del tasks[idx]
...
"""
Note:
- This is a DEMO implementation using in-memory Python dicts as the database.
